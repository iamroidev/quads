...
import { ScrollView, StyleSheet, Text, TextInput, TouchableOpacity, View, Linking, Alert, ActivityIndicator } from 'react-native';
...
const ARTICLES = {
  help: [
    {
      ...
      content: 'Welcome to the official QUADS platform for UMaT. Use your institutional email (@student.umat.edu.gh or @st.umat.edu.gh) to register. After login, visit the Security Center to verify your account. You can browse listings on the Board or switch to Seller mode to manage your own store.'
    },
    {
  ...
  safety: [
    ...
    },
    {
      ...
      content: 'Use the Report button for fraudulent listings. In emergencies, contact UMaT Campus Security. Our moderation team reviews all reports within 24 hours.'
    }
  ]
};
...
const SupportScreen = ({ navigation }: any) => {
      ...
      if (res.success && res.data?.userId) {
        ...
        if (convRes.success && convRes.data?.conversation) {
          navigation.navigate('Chat', {
            ...
            otherUser: { _id: res.data.userId, name: 'QUADS AI Assistant' },
          });
        }
      } else {
        throw new Error('AI Support bot not configured on server');
      }
    } catch (err) {
      console.warn('Error starting AI support conversation:', err);
      Alert.alert(
        'AI Assistant Offline',
        'Our smart support bot is busy assisting other students. Please try again or use email/WhatsApp support.'
      );
    } finally {
      setLoadingAi(false);
    }
  ...
  const getFiltered = (list: SupportArticle[], category: string) => {
  ...
  const helpArticles = getFiltered(ARTICLES.help, 'help');
  const safetyArticles = getFiltered(ARTICLES.safety, 'safety');
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      <ScrollView contentContainerStyle={styles.content}>
        <ScreenHeader
          eyebrow="HELP CENTER"
          title="Support Desk"
          subtitle="Campus support guides, safe buying, and contact protocols."
        />

        {/* 🤖 QUADS AI assistant call-to-action */}
        <TouchableOpacity
          style={styles.aiCard}
          onPress={startAiChat}
          disabled={loadingAi}
        >
          <View style={styles.aiHeader}>
            <Ionicons name="desktop-outline" size={20} color="#8a5a00" />
            <Text style={styles.aiTitle}>🤖 QUADS AI SUPPORT ASSISTANT</Text>
          </View>
          <Text style={styles.aiText}>
            Need help with escrow holdings, payment confirmations, or listing guidelines? Chat instantly with our institutional bot.
          </Text>
          <View style={styles.aiActionRow}>
            {loadingAi ? (
              <ActivityIndicator color="#fff" size={16} />
            ) : (
              <Text style={styles.aiActionText}>START LIVE ASSISTANCE →</Text>
            )}
          </View>
        </TouchableOpacity>
        ...
        <View style={styles.quickContacts}>
          <TouchableOpacity
            style={[styles.contactCard, { backgroundColor: '#e0f2f7' }]}
            onPress={() => openLink('https://wa.me/233551500736')}
          >
            <Ionicons name="logo-whatsapp" size={28} color="#0284c7" style={styles.cardIcon} />
            <Text style={styles.cardTitle}>WhatsApp Support</Text>
            <Text style={styles.cardSub}>Quick support via WhatsApp chat</Text>
          </TouchableOpacity>

          <TouchableOpacity
            style={[styles.contactCard, { backgroundColor: '#ffe4e4' }]}
            onPress={() => openLink('mailto:support@quadsmarket.tech')}
          >
            <Ionicons name="mail-outline" size={28} color="#ff6b6b" style={styles.cardIcon} />
        ...
        {/* Institutional Safety Card */}
        <View style={styles.institutionalCard}>
          <Text style={styles.institutionalTitle}>🛡️ UMaT CAMPUS SAFETY ZONES</Text>
          <Text style={styles.institutionalText}>
            Always meet campus sellers inside public nodes. Highly recommended places include the UMaT Library Foyer, Student Center Lobby, or Hall Common Rooms. Never meet off-campus or late at night.
          </Text>
        </View>
      </ScrollView>
    </SafeAreaView>
